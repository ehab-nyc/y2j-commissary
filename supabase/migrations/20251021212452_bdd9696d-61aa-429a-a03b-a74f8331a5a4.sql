...
DROP POLICY IF EXISTS "System can access settings" ON public.system_settings;
...
-- Update the notify_customer_order_status function to send SMS
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    IF NEW.status = 'completed' THEN
      ...
      IF customer_profile.sms_notifications = true AND customer_profile.phone IS NOT NULL AND customer_profile.phone != '' THEN
