-- ===========================================
-- SECURITY FIX: SMS Authentication & Trigger Security (Fixed)
-- ===========================================

-- Fix 1: Restrict system_settings access (addresses hardcoded credentials issue)
-- Drop all existing policies first
DROP POLICY IF EXISTS "System can access settings" ON system_settings;
DROP POLICY IF EXISTS "System functions can read settings" ON system_settings;
DROP POLICY IF EXISTS "Super admins can manage settings" ON system_settings;

-- Only allow SECURITY DEFINER functions to read settings (no direct user access)
CREATE POLICY "System functions can read settings"
ON system_settings FOR SELECT
USING (auth.uid() IS NULL);  -- Only SECURITY DEFINER functions (no user context)

-- Allow super_admins to manage settings
...
-- Fix 2: Update triggers to use create_notification function instead of direct INSERT
-- This addresses the DEFINER_OR_RPC_BYPASS security issue

-- Update notify_staff_new_order to use create_notification
...
-- Update notify_customer_order_status to use create_notification
...
-- Update check_low_stock to use create_notification
...
-- Update trigger_check_low_stock to use create_notification
